...
- **审阅结果导出**：仅导出已处理且未标记失败的 episode，并应用保存的裁剪区间；
- **原始数据直转**：忽略人工失败标记、处理状态和裁剪区间，将所有结构校验通过的 episode 按完整时长直接转换。缺少必需文件或无法同步的 episode 不能生成有效 LeRobot 数据，不会被纳入。

“审阅结果导出”模式会同步当前 raw 目录中所有“已处理且未失败”的 episode，包括以前导出过但后来被重新裁剪的 episode：
